...
| KPI | Definition |
|-----|-----------|
| **Leads** | `COUNT(DISTINCT lead_id)` from `customer_leads_long` (excludes org `5000`, the Automall buyer channel — see the org-`5000` note below). `leads_without_walkins` excludes pop-up/walk-in leads. |
