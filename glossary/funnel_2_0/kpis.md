...
| **Total Reservations** | Under approval | Total number of reservation items (sum of item quantity) created during the selected analysis period for the chosen brand. | SAP | Sum ITEM QUANTITY for reservation items where ORDER ITEM CREATION DATE is within the selected analysis period and ORDER TYPE IN ('Standard Order', 'Fleet Order', 'AFM Corporate Order'). | Current implementation in Funnel 2.0. Not used by BYD (available in BYD Dashboard legacy and Funnel 2.0). Order types map to SAP codes ZOR / YOR / TA (Standard / Fleet / AFM Corporate). `item_quantity` is a line flag, so SUM counts reservation order line items. No `sales_group` filter — includes retail, fleet and corporate. See the proposed active-orders alternative below. |
| **Total Reservation (Proposed — active orders)** | Proposed — for discussion · not yet implemented | Sum of reservation item quantities for active reservations created during the selected analysis period — reservations that have not been rejected and whose latest secondary status is 'Sales Order Created' (or blank). Applies the same validity filters as Total Open Reservations, but as a flow over the analysis period rather than an as-of snapshot. | SAP | Sum ITEM QUANTITY for reservation items where ORDER CREATION DATE (`sales_order_creation_date`) is within the selected analysis period, ORDER TYPE IN ('Standard Order', 'Fleet Order', 'AFM Corporate Order'), REASON FOR REJECTION (`rejection_reason_text`) is blank, and the latest SECONDARY STATUS (`secondary_status_description_latest`) is 'Sales Order Created' or blank. | Proposed alternative to the current Total Reservations KPI, raised for discussion. Keeps the same item-quantity sum but restricts it to active / valid reservations by adding the Total Open Reservations validity filters (no reason for rejection; latest secondary status 'Sales Order Created' or blank) and anchoring on order creation date. Unlike Total Open Reservations it is a period flow, not an as-of snapshot, and it does not apply the '0020' / '0060' order-number exclusion or duplicate-VIN removal. Not implemented. |
...
| **Total Reservation (Proposed — active orders)** | Reservation order line item. `item_quantity` is a =1 line flag, so the SUM is a count of active line items — same grain as Total Reservations. | Order creation date (`sales_order_creation_date`). | Order type ZOR / YOR / TA; not rejected (`rejection_reason_text` is null); latest secondary status 'Sales Order Created' or null. Filtered to the selected brand / sales org. |
...
| **Total Reservation (Proposed — active orders)** | `PAD_100_sales_document_header`, `PAD_100_sales_document_item_data` | `sales_ordr_vn_d` | — | — |
